...
export class MultiplayerGame{
 ...
 private get active(){return !this.hud.menuOpen&&this.hud.alive&&!this.hud.result&&this.connection.phase==='playing';}
 ...
 private readonly onKeyUp=(event:KeyboardEvent)=>{this.keys.delete(event.code);this.sendInput();};
 ...
 private tryFire(now:number){
  ...
  if(this.hud.ammo<=0){this.reload();return;}if(this.keys.has('ShiftLeft')&&!this.presentation.aim)return;
 ...
 private movementInput():MovementInput&{pitch:number}{const enabled=this.active;return {moveX:enabled?Number(this.keys.has('KeyD'))-Number(this.keys.has('KeyA')):0,moveZ:enabled?Number(this.keys.has('KeyW'))-Number(this.keys.has('KeyS')):0,yaw:this.yaw,pitch:this.pitch,jump:enabled&&this.keys.has('Space'),crouch:enabled&&(this.keys.has('KeyC')||this.keys.has('ControlLeft')),sprint:enabled&&this.keys.has('ShiftLeft'),aiming:this.presentation.aim};}
 private sendInput(){if(this.connection.phase!=='playing'){this.unsent=[];return;}while(this.unsent.length)this.connection.send({type:'input_batch',commands:this.unsent.splice(0,16).map(command=>({type:'input',...command}))});}
 private applySnapshot(message:Extract<ServerMessage,{type:'snapshot'}>){
  ...
  if(!this.initialized||this.life!==local.life){this.predictor.reset(local);this.unsent=[];this.accumulator=0;this.life=local.life;this.initialized=true;this.presentation.reset();this.predictedShots=[];}
 ...
 private renderRemoteActors(){
  ...
  for(const entity of this.latest){if(entity.id===this.playerId)continue;let actor=this.actors.get(entity.id);if(!actor){actor=makeSoldier(this.actors.size);this.actors.set(entity.id,actor);this.scene.add(actor.root);}
   const a=sampled.before.entities.find(e=>e.id===entity.id)??entity,b=sampled.after.entities.find(e=>e.id===entity.id)??entity;
   const pose=interpolatePose(a,b,sampled.alpha,a.life!==b.life||a.alive!==b.alive);actor.root.position.set(pose.x,pose.y,pose.z);actor.root.rotation.y=pose.yaw;actor.root.visible=entity.alive;
  }
 ...
 private frame(){
  ...
  const pose=this.presentation.update(dt,{reloadLeft:this.localReloadLeft,reloadTotal:WEAPONS[this.weapon].reload,alive:this.hud.alive,blocked:this.hud.menuOpen,sprint:this.keys.has('ShiftLeft')});
  if(this.initialized&&this.connection.phase==='playing'&&this.hud.alive&&!this.hud.result){
   this.accumulator+=dt;
  ...
  this.renderRemoteActors();this.camera.updateMatrixWorld(true);this.tryFire(now);this.shotEffects=updateShotEffects(this.scene,this.shotEffects,dt);this.renderer.render(this.scene,this.camera);
  this.publishElapsed+=dt;if(this.publishElapsed>=.05){this.publishElapsed=0;const eye={x:preview.position.x,y:this.camera.position.y,z:preview.position.z};this.emit({x:eye.x,z:eye.z,yaw:this.yaw,aiming:pose.aiming,reloadLeft:this.localReloadLeft,connected:this.connection.phase==='playing'||this.connection.phase==='ended',enemies:this.latest.filter(e=>e.id!==this.playerId).map(e=>({x:e.x,z:e.z,visible:e.alive&&Math.hypot(e.x-eye.x,e.z-eye.z)<33&&lineClear(eye,{x:e.x,y:e.y+1.2,z:e.z})}))});}
 }
 dispose(){this.disposed=true;clearInterval(this.inputTimer);this.unsubscribe();this.renderer.setAnimationLoop(null);removeEventListener('resize',this.resize);removeEventListener('keydown',this.onKeyDown);removeEventListener('keyup',this.onKeyUp);removeEventListener('mousemove',this.onMouseMove);removeEventListener('mouseup',this.onMouseUp);removeEventListener('blur',this.onBlur);document.removeEventListener('pointerlockchange',this.onPointerLock);this.renderer.domElement.removeEventListener('mousedown',this.onMouseDown);this.renderer.domElement.removeEventListener('contextmenu',this.onContextMenu);if(document.pointerLockElement===this.renderer.domElement)document.exitPointerLock();this.shotEffects=updateShotEffects(this.scene,this.shotEffects,Infinity);this.scene.traverse(object=>{if(object instanceof THREE.Mesh){object.geometry.dispose();for(const material of Array.isArray(object.material)?object.material:[object.material])material.dispose();}});this.audio.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
}
