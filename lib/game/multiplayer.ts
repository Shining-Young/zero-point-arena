import {nextShotTime} from './fire-timing.ts';
...
import type { ShopState,ShopAction } from '../../shared/economy.ts';
import { buildWorld,makeWeapon,animateWeapon,makeSoldier,setSoldierWeapon } from './world.ts';

export type MultiplayerHud=ShopState&{
 health:number;ammo:number;reserve:number;kills:number;deaths:number;remaining:number;latency:number;connected:boolean;
 weapon:WeaponId;reloadLeft:number;alive:boolean;protection:number;aiming:boolean;hitMarker:number;headshot:boolean;hurt:number;result:'win'|'loss'|'draw'|null;
 shopOpen?:boolean;shopMessage?:string;shotHeat?:number;crosshairScale?:number;
...
export const INITIAL_HUD:MultiplayerHud={coins:0,inventory:{pistol:{ammo:12,reserve:0}},primary:null,canBuy:false,buyBlockedReason:'等待比赛同步',health:100,ammo:12,reserve:0,kills:0,deaths:0,remaining:300,latency:0,connected:false,weapon:'pistol',reloadLeft:0,alive:true,protection:0,aiming:false,hitMarker:0,headshot:false,hurt:0,result:null,menuOpen:true,leaderKills:0,leaderName:'对手',x:-19,z:19,yaw:0,enemies:[],feed:[],respawnLeft:0};
const STEP=.01;
...
export class MultiplayerGame{
 ...
 private shotHeat=0;private purchaseId?:string;
 private sequence=0;private unsent:MovementCommand[]=[];private weapon:WeaponId='pistol';private presentation=new WeaponPresentation();
 ...
 private get active(){return !this.hud.menuOpen&&!this.hud.shopOpen&&this.hud.alive&&!this.hud.result&&this.connection.phase==='playing'&&!this.connection.recovering;}
 private readonly onKeyDown=(event:KeyboardEvent)=>{
  if(event.code==='KeyB'&&!event.repeat&&!this.hud.result){event.preventDefault();if(this.hud.shopOpen)this.resume();else this.openShop();return;}
  if(event.code==='Escape'){event.preventDefault();if(this.hud.shopOpen)this.resume();else this.openSettings();return;}
  ...
  if(event.code==='Digit1'||event.code==='Digit2'){
   ...
   this.connection.send({type:'switch_weapon',weapon:event.code==='Digit1'?(this.hud.primary??'pistol'):'pistol'});
  }
 };
 ...
 private readonly onPointerLock=()=>{if(document.pointerLockElement!==this.renderer.domElement&&!this.hud.shopOpen)this.openSettings();};

 constructor(private readonly container:HTMLElement,private readonly connection:GameConnection&{paused?:boolean;setPaused?:(paused:boolean)=>void},private readonly playerId:string,private readonly callback:(hud:MultiplayerHud)=>void){
  ...
  this.guns=WEAPON_IDS.map(makeWeapon);for(const gun of this.guns){gun.scale.setScalar(.7);this.gunRig.add(gun);}this.setWeapon('pistol');
  ...
  this.unsubscribe=connection.subscribe(message=>{
   if(message.type==='welcome'){this.purchaseId=undefined;this.initialized=false;this.unsent=[];this.predictor.pending=[];this.snapshotBuffer=[];this.accumulator=0;this.predictedShots=[];this.openSettings();}
   ...
   if(message.type==='shop_result'&&message.requestId===this.purchaseId){this.purchaseId=undefined;this.emit({shopMessage:message.ok?'操作成功':message.reason});}
 ...
 openSettings(){this.connection.setPaused?.(true);this.keys.clear();this.shooting=false;this.presentation.reset();this.sendInput();this.emit({menuOpen:true,shopOpen:false,aiming:false});if(document.pointerLockElement===this.renderer.domElement)document.exitPointerLock();}
 openShop(){if(this.hud.result)return;this.connection.setPaused?.(false);this.keys.clear();this.shooting=false;this.presentation.reset();this.sendInput();this.emit({shopOpen:true,menuOpen:false,shopMessage:''});if(document.pointerLockElement===this.renderer.domElement)document.exitPointerLock();}
 buy(action:ShopAction,weapon:WeaponId){if(!this.hud.shopOpen||this.purchaseId)return;const requestId=crypto.randomUUID();this.purchaseId=requestId;this.emit({shopMessage:'正在处理…'});this.sendInput();if(!this.connection.send({type:'shop',requestId,action,weapon})){this.purchaseId=undefined;this.emit({shopMessage:'连接恢复后再购买'});}}
 resume(){if(this.hud.result)return;this.keys.clear();this.shooting=false;this.audio.init();void this.renderer.domElement.requestPointerLock().then(()=>{if(!this.disposed){this.connection.setPaused?.(false);this.emit({menuOpen:false,shopOpen:false});}}).catch(()=>this.openSettings());}
 ...
 private setWeapon(weapon:WeaponId){if(this.weapon!==weapon){this.presentation.reset();this.localReloadLeft=0;this.lastFire=performance.now();}this.weapon=weapon;this.presentation.configure(weapon);this.guns.forEach((gun,index)=>gun.visible=WEAPON_IDS[index]===weapon);}
 private reload(){const w=WEAPONS[this.weapon];if(!this.active||this.localReloadLeft>0||this.hud.ammo>=w.capacity||(!w.unlimitedAmmo&&this.hud.reserve<=0))return;this.localReloadLeft=w.reload;this.reloadRequested=performance.now();this.reloadConfirmed=false;this.connection.send({type:'reload'});this.audio.reload();}
 private tryFire(now:number){
  if(!this.active||!this.shooting||this.localReloadLeft>0||now-this.lastFire<WEAPONS[this.weapon].cadence*1000)return;
  if(this.hud.ammo<=0){this.reload();if(!WEAPONS[this.weapon].automatic)this.shooting=false;return;}if(this.movementInput().sprint)return;
  this.sendInput();this.lastFire=nextShotTime(this.lastFire,now,WEAPONS[this.weapon].cadence*1000);const sequence=++this.sequence;this.predictedShots.push(sequence);
  this.showLocalShot(sequence);this.shotHeat+=1;this.presentation.shoot();this.audio.shot(false,this.weapon);if(!WEAPONS[this.weapon].automatic)this.shooting=false;this.emit({ammo:Math.max(0,this.hud.ammo-1)});
  this.connection.send({type:'fire',weapon:this.weapon,sequence,yaw:this.camera.rotation.y,pitch:Math.max(-1.4,Math.min(1.4,this.camera.rotation.x)),clientTime:Date.now(),inputSequence:this.lastInputSequence,life:this.life});
 }
 private showLocalShot(sequence:number){
  ...
  const movement=this.movementInput(),shots=traceWeaponBurst(eye,this.camera.rotation.y,this.camera.rotation.x,this.weapon,this.presentation.aim,targets,sequence,Boolean(movement.moveX||movement.moveZ),movement.crouch,this.shotHeat);
  for(const shot of shots)this.shotEffects.push(spawnShotEffect(this.scene,new THREE.Vector3(shot.start.x,shot.start.y,shot.start.z),new THREE.Vector3(shot.end.x,shot.end.y,shot.end.z)));
 }
 ...
 private applySnapshot(message:Extract<ServerMessage,{type:'snapshot'}>){
  ...
  this.predictedShots=this.predictedShots.filter(sequence=>sequence>(local.lastShot??-1));this.shotHeat=(local.shotHeat??0)+this.predictedShots.length;
  const leader=message.entities.filter(e=>e.id!==this.playerId).sort((a,b)=>b.kills-a.kills)[0];
  this.emit({coins:local.coins??0,inventory:local.inventory??{},primary:local.primary??null,canBuy:local.canBuy??false,buyBlockedReason:local.buyBlockedReason??'',health:Math.ceil(local.health),ammo:Math.max(0,local.ammo-this.predictedShots.length),reserve:local.reserve,kills:local.kills,deaths:local.deaths,remaining:message.remainingSeconds,latency:this.connection.latency,connected:this.connection.phase==='playing',weapon:local.weapon,alive:local.alive,protection:local.spawnProtection,leaderKills:leader?.kills??0,leaderName:leader?.nickname??'对手',respawnLeft:local.respawnLeft});
 }
 private renderRemoteActors(){
  ...
  for(const entity of this.latest){if(entity.id===this.playerId)continue;let actor=this.actors.get(entity.id);if(!actor){actor=makeSoldier(this.actors.size);this.actors.set(entity.id,actor);this.scene.add(actor.root);this.feedback.set(entity.id,new ActorFeedback(actor.root,this.container,entity.nickname,entity.isBot));}
   ...
   const pose=interpolatePose(a,b,sampled.alpha,a.life!==b.life||a.alive!==b.alive);actor.root.position.set(pose.x,pose.y,pose.z);actor.root.rotation.y=pose.yaw;actor.root.visible=entity.alive;setSoldierWeapon(actor,entity.weapon);actor.root.scale.y=entity.crouched?.65:1;this.feedback.get(entity.id)?.update(this.camera,entity.alive,entity.spawnProtection,entity.crouched);
  }
 ...
 private applyCombatEvent(message:Extract<ServerMessage,{type:'combat_event'}>){
  if(message.event==='shot'){if(message.actorId!==this.playerId){const w=this.latest.find(e=>e.id===message.actorId)?.weapon??'pistol';this.audio.shot(true,w);for(const end of message.ends??[message.end])this.showRemoteShot(message.actorId,message.yaw,message.pitch,end);}return;}
 ...
 private frame(){
  ...
  this.localReloadLeft=Math.max(0,this.localReloadLeft-dt);this.shotHeat=decayShotHeat(this.shotHeat,this.weapon,dt);
  const snapshotAge=Math.max(0,Date.now()-this.connection.lastSnapshotAt);
  if(this.initialized&&!this.connection.paused&&this.connection.phase==='playing'&&!this.hud.result&&(snapshotAge>750||this.predictor.pending.length>=90)){this.connection.resyncInputs();this.unsent=[];this.accumulator=0;this.keys.clear();this.shooting=false;this.presentation.reset();}
  const pose=this.presentation.update(dt,{reloadLeft:this.localReloadLeft,reloadTotal:WEAPONS[this.weapon].reload,alive:this.hud.alive,blocked:this.hud.menuOpen||this.hud.shopOpen,sprint:this.movementInput().sprint});
  if(this.initialized&&!this.connection.paused&&this.connection.phase==='playing'&&!this.connection.recovering&&this.hud.alive&&!this.hud.result){
   this.accumulator+=dt;
  ...
  this.camera.position.set(preview.position.x,preview.movement.y+(preview.movement.crouched?1.05:1.68),preview.position.z);this.camera.rotation.order='YXZ';this.camera.rotation.set(Math.max(-1.4,Math.min(1.4,this.pitch+this.presentation.recoil*.6)),this.yaw,0);this.camera.fov=pose.fov;this.camera.updateProjectionMatrix();
  this.gunRig.position.set(pose.x,pose.y,pose.z);this.gunRig.rotation.set(pose.rotationX,0,pose.rotationZ);this.gunRig.visible=!(this.weapon==='sniper'&&pose.aiming);animateWeapon(this.guns[WEAPON_IDS.indexOf(this.weapon)],pose.reloadProgress,Math.min(1,(now-this.lastFire)/(WEAPONS[this.weapon].cadence*1000)));
