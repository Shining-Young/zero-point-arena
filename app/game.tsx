  ...
  return <main className={`game-shell phase-${state.phase}`}>
    ...
    {menu&&<>
      <section className="briefing">
        ...
        <button className="text-button" onClick={onBack}><ArrowLeft size={16}/> 返回模式选择</button>
    ...
    {!menu&&<>
      ...
      {state.fallback&&playing&&<div className="fallback-note"><span>当前窗口禁止锁定鼠标，移动鼠标仍可转向；建议在独立浏览器打开。</span><button onClick={()=>game.current?.requestMouseLock()}>再次锁定鼠标</button></div>}
    </>}
