...
import type { RoomPlayer,ServerMessage } from '../shared/protocol.ts';
...
function Lobby({room,playerId,connection,onLeave}:{room:Extract<ServerMessage,{type:'room_state'}>;playerId:string;connection:GameConnection;onLeave:()=>void}){
  const me=room.players.find(p=>p.playerId===playerId),host=room.hostPlayerId===playerId,allReady=room.players.filter(p=>!p.isBot&&p.connected).every(p=>p.ready);
  return <><div className="room-code"><span>房间码</span><strong>{room.roomCode}</strong><button onClick={()=>navigator.clipboard?.writeText(room.roomCode)}>复制</button></div><div className="player-list">{room.players.map((player:RoomPlayer)=><div key={player.playerId}><span>{player.isHost?'★ ':''}{player.nickname}</span><b>{player.connected?(player.ready?'已准备':'未准备'):'已掉线'}</b></div>)}</div>{host&&<div className="room-options"><label>人机补位<select value={room.botCount} onChange={e=>connection.send({type:'configure_room',botCount:Number(e.target.value),difficulty:room.difficulty})}>{[0,1,2,3].map(n=><option key={n} value={n}>{n} 名</option>)}</select></label><label>难度<select value={room.difficulty} onChange={e=>connection.send({type:'configure_room',botCount:room.botCount,difficulty:e.target.value as 'easy'|'normal'|'hard'})}><option value="easy">新兵</option><option value="normal">标准</option><option value="hard">精英</option></select></label></div>}<button className="primary-button" onClick={()=>connection.send({type:'set_ready',ready:!me?.ready})}>{me?.ready?'取消准备':'准备'}</button>{host&&<button className="primary-button secondary" disabled={!allReady} onClick={()=>connection.send({type:'start_match'})}>开始比赛</button>}<button className="text-button" onClick={onLeave}>离开房间</button></>;
}
