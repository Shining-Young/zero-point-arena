...
import type {ShopAction} from '../shared/economy.ts';
...
export function MultiplayerMatch({hud,roomCode,settings,isHost,onSettings,onOpen,onResume,onReturnToRoom,onLeave,onShop,onBuy,training=false}:{training?:boolean;onShop:()=>void;onBuy:(action:ShopAction,weapon:WeaponId)=>void;hud:DisplayHud;roomCode:string;settings:GameSettings;isHost:boolean;onSettings:(value:GameSettings)=>void;onOpen:()=>void;onResume:()=>void;onReturnToRoom:()=>void;onLeave:()=>void}){
  const time=`${String(Math.floor(hud.remaining/60)).padStart(2,'0')}:${String(Math.floor(hud.remaining%60)).padStart(2,'0')}`;
  const weapon=WEAPONS[hud.weapon],reloadTotal=weapon.reload;
  return <>
    <header className="topbar"><div className="brand"><span className="brand-symbol"><Crosshair size={25}/></span><span>ZERO / POINT<small>{training?'单人训练':`联机 · ${roomCode}`}</small></span></div><div className="scoreboard"><div className="your-score"><span>你</span><strong>{String(hud.kills).padStart(2,'0')}</strong></div><div className="match-clock"><small>击杀竞赛 · 目标 15</small><b>{time}</b></div><div className="enemy-score"><strong>{String(hud.leaderKills).padStart(2,'0')}</strong><span>{hud.leaderName||'领先对手'}</span></div></div><div className="top-actions"><button aria-label={settings.sound?'关闭音效':'打开音效'} onClick={()=>onSettings({...settings,sound:!settings.sound})}>{settings.sound?<Volume2 size={19}/>:<VolumeX size={19}/>}</button><button className="pause-button" onClick={onOpen}>ESC</button></div></header>
    ...
    {hud.alive&&!hud.menuOpen&&!hud.shopOpen&&!hud.result&&<><div className={`crosshair ${hud.aiming?'aiming':''}`} style={{transform:`translate(-50%,-50%) scale(${hud.crosshairScale??1})`,display:hud.weapon==='sniper'&&hud.aiming?'none':undefined}}><i/><i/><i/><i/><b/></div>{hud.weapon==='sniper'&&hud.aiming&&<div className="sniper-scope"><b/></div>}{hud.hitMarker>0&&<div key={hud.hitMarker} className={`multi-hit-marker ${hud.headshot?'headshot':''}`}>×</div>}{hud.protection>0&&<div className="protection"><Shield size={15}/> 出生保护 {Math.ceil(hud.protection)}s</div>}{hud.reloadLeft>0&&<div className="reload-indicator"><span>正在换弹</span><div><i style={{width:`${Math.max(0,Math.min(100,(1-hud.reloadLeft/reloadTotal)*100))}%`}}/></div></div>}{hud.ammo===0&&hud.reloadLeft<=0&&<div className="reload-indicator">{(weapon.unlimitedAmmo||hud.reserve>0)?'按 R 换弹':'备用弹药耗尽 · 按 1 / 2 切换武器'}</div>}</>}
    ...
    <div className="bottom-hud"><div className={`health-block ${hud.health<30?'low':''}`}><Cross size={27}/><strong>{hud.health}</strong><div><span>生命值</span><div className="health-track"><i style={{width:`${hud.health}%`}}/></div></div></div><div className="hud-tip"><kbd>1</kbd> 主武器 <kbd>2</kbd> 手枪 <span>·</span><kbd>R</kbd> 换弹</div><div className="ammo-block"><div className="weapon-label"><span>{weapon.name}</span><small>{weapon.automatic?'自动射击':'点按射击'}</small></div><div className="ammo-count"><strong className={hud.ammo<6?'low-ammo':''}>{String(hud.ammo).padStart(2,'0')}</strong><span>/ {weapon.unlimitedAmmo?'∞':hud.reserve}</span></div></div></div>
    ...
    {hud.menuOpen&&!hud.result&&<div className="modal-backdrop"><section className="game-panel"><div className="eyebrow">MATCH SETTINGS</div><h2>行动设置<span>.</span></h2><p>{training?'训练已暂停，准备好后继续。':'联机比赛仍在继续。准备好后返回战场。'}</p><SensitivityControl value={settings.sensitivity} onChange={sensitivity=>onSettings({...settings,sensitivity})}/><div className="setting-row sound-setting"><span>游戏音效</span><Switch checked={settings.sound} onCheckedChange={sound=>onSettings({...settings,sound})} aria-label="游戏音效"/></div><button className="primary-button" onClick={onResume}><Play size={18}/> 继续行动</button><button className="text-button" onClick={onLeave}>{training?'返回模式选择':'离开房间，返回模式选择'}</button><NetworkDetails hud={hud}/><p>WASD 移动 · 左键射击 · 右键切换瞄准<br/>R 换弹 · 1 / 2 切枪 · B 商店 · ESC 设置</p></section></div>}
    {hud.result&&<div className="modal-backdrop"><section className="game-panel result-panel"><div className="eyebrow">MISSION COMPLETE</div><h2>{hud.result==='win'?'行动胜利':hud.result==='loss'?'再接再厉':hud.result==='draw'?'势均力敌':'比赛已结束'}<span>.</span></h2><p>{hud.result==='unknown'?'已重新连接到结束的房间，本局结果不可用。':'本局比赛已结束'}</p>{hud.result!=='unknown'&&<div className="result-stats"><div><b>{hud.kills}</b><span>击杀</span></div><div><b>{hud.deaths}</b><span>阵亡</span></div><div><b>{Math.round(300-hud.remaining)}s</b><span>对战时长</span></div></div>}{isHost?<button className="primary-button" onClick={onReturnToRoom}>{training?'重新训练':'返回房间'}</button>:<p className="room-waiting">等待房主返回房间…</p>}<button className="text-button" onClick={onLeave}>离开房间，返回模式选择</button></section></div>}
    {!hud.result&&<button className="coins-hud" onClick={onShop}>金币 {hud.coins} · B 商店</button>}
    {hud.shopOpen&&!hud.result&&<Shop state={hud} message={hud.shopMessage} onBuy={onBuy} onClose={onResume}/>}
