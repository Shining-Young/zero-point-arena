...
test('acknowledges only simulated input and does not repeat a completed command',()=>{
 const sim=new RifleFixture([{id:'a',nickname:'A',x:8,z:0}]);
...
test('queued shot uses its exact command boundary and precedes a subsequent reload',()=>{
 ...
 sim.tick(.1);
 const shot=sim.events.find(e=>e.type==='shot');assert.ok(shot);const expected=traceWeaponBurst({x:8,y:1.68,z:-.23},Math.PI/2,0,'rifle',false,[],6,true)[0].end;assert.ok(Math.hypot(shot.end.x-expected.x,shot.end.y-expected.y,shot.end.z-expected.z)<1e-9);
 assert.equal(sim.player('a').ammo,29);assert.ok(sim.player('a').reloadLeft>0);
 assert.ok(sim.player('a').z<-.23);
});

test('prediction converges at 80, 200 and 400 ms round trip after releasing movement',()=>{
 for(const rtt of [80,200,400]){
  const sim=new RifleFixture([{id:'a',nickname:'A',x:8,z:0}]),prediction=new motion.CommandPrediction();prediction.reset(sim.player('a'));
