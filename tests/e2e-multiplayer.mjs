...
try{
  a.send({type:'hello',protocolVersion:5,releaseVersion:'0.3.0'});b.send({type:'hello',protocolVersion:5,releaseVersion:'0.3.0'});
  ...
  a.send({type:'fire',weapon:'pistol',sequence:2,yaw:0,pitch:0,clientTime:Date.now()});await a.wait('combat_event');await new Promise(resolve=>setTimeout(resolve,120));
  const fired=[...a.messages].reverse().find(message=>message.type==='snapshot').entities.find(entity=>entity.id===local.id);assert.equal(fired.ammo,11);
