...
test('accepts the core client messages', () => {
  assert.equal(parse({ type: 'hello', protocolVersion: 1, releaseVersion: '1.0.0' }).type, 'hello');
...
test('rejects an unsupported protocol version', () => {
  assert.throws(() => parse({ type: 'hello', protocolVersion: 2, releaseVersion: '1.0.0' }), error => error instanceof ProtocolError && error.code === 'VERSION_MISMATCH');
});
...
test('rejects messages larger than the UTF-8 byte limit', () => {
  const raw = JSON.stringify({ type: 'hello', protocolVersion: 1, releaseVersion: '测'.repeat(MAX_MESSAGE_BYTES) });
