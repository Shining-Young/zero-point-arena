...
test('accepts the core client messages', () => {
  assert.equal(parse({ type: 'hello', protocolVersion:5, releaseVersion: '1.0.0' }).type, 'hello');
...
test('rejects messages larger than the UTF-8 byte limit', () => {
  const raw = JSON.stringify({ type: 'hello', protocolVersion:5, releaseVersion: '测'.repeat(MAX_MESSAGE_BYTES) });
