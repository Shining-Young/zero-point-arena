...
test('reports application readiness and rejects other websocket paths', async t => {
  ...
  assert.deepEqual(await response.json(), { ok: true, protocolVersion:5, releaseVersion: '0.3.0' });
...
test('requires hello then creates, joins and broadcasts a room', async t => {
  ...
  a.send({ type: 'hello', protocolVersion:5, releaseVersion: '0.3.0' });
  a.send({ type: 'create_room', nickname: 'A', humanLimit: 2, botCount: 0, difficulty: 'easy' });
  const room = await a.waitFor('room_state');
  b.send({ type: 'hello', protocolVersion:5, releaseVersion: '0.3.0' });
...
test('starts a match and publishes authoritative snapshots', async t => {
  ...
  const a = await opened(`${server.wsBase}/game`), b = await opened(`${server.wsBase}/game`);
  t.after(() => { a.ws.close(); b.ws.close(); });
  for (const c of [a,b]) c.send({ type:'hello', protocolVersion:5, releaseVersion:'0.3.0' });
  ...
  a.send({ type:'fire', weapon:'pistol', sequence:1, yaw:.75, pitch:-.2, clientTime:Date.now() });
...
test('network batches preserve shot pose, action order and final stopped position',async t=>{
 ...
 a.send({type:'fire',sequence:6,weapon:'pistol',yaw:Math.PI/2,pitch:0,inputSequence:5,life:me.life,clientTime:Date.now()});
 ...
 assert.deepEqual(events.map(e=>e.event),['shot','reload']);assert.ok(Math.abs(events[0].end.z-(me.z-.23))<.8);
 const snapshot=a.messages.filter(m=>m.type==='snapshot').at(-1),local=snapshot.entities.find(e=>e.id===me.id);
 assert.equal(snapshot.lastProcessedInput,11);assert.equal(local.ammo,11);assert.ok(local.reloadLeft>0);assert.ok(Math.abs(local.z-(me.z-.23))<1e-8);
...
test('responds to heartbeat without a room and explicitly reports input throttling',async t=>{
 ...
 a.send({type:'hello',protocolVersion:5,releaseVersion:'0.3.0'});a.send({type:'ping',clientTime:123});assert.equal((await a.waitFor('pong')).clientTime,123);
 ...
 for(let n=0;n<65;n++)a.send({type:'input_batch',commands:[{type:'input',sequence:n,moveX:0,moveZ:1,yaw:0,pitch:0,jump:false,crouch:false,sprint:false,dt:.01,life:0,clientTime:Date.now()}]});
...
test('replacement socket stays connected when the evicted socket closes later',async t=>{
 ...
 old.send({type:'hello',protocolVersion:5,releaseVersion:'0.3.0'});old.send({type:'create_room',nickname:'A',humanLimit:2,botCount:0,difficulty:'normal'});const identity=await old.waitFor('welcome'),room=await old.waitFor('room_state');
 const replacement=await opened(server.wsBase+'/game');t.after(()=>replacement.ws.close());replacement.send({type:'hello',protocolVersion:5,releaseVersion:'0.3.0',reconnectToken:identity.reconnectToken});await replacement.waitFor('welcome');await new Promise(r=>setTimeout(r,60));
