...
test('rejects stale input and enforces weapon cadence and ammo', () => {
  const sim = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -10 }], { now: () => 1000 });
...
test('reloads on the server after the weapon delay', () => {
  const sim = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -10 }]);
  sim.player('a').ammo = 1;
...
test('rejects shots through cover and damages visible targets', () => {
  const covered = new RifleFixture([{ id: 'a', nickname: 'A', x: -9, z: 3 }, { id: 'b', nickname: 'B', x: -9, z: -18 }], { now: () => 4000 });
  covered.tick(3.1);
  covered.fire('a', { sequence: 1, weapon: 'rifle', yaw: 0, pitch: -0.08, clientTime: 4000 });
  assert.equal(covered.player('b').health, 100);
  const open = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -8 }, { id: 'b', nickname: 'B', x: 8, z: -15 }], { now: () => 4000 });
  open.tick(3.1);
  open.fire('a', { sequence: 1, weapon: 'rifle', yaw: 0, pitch: -0.08, clientTime: 4000 });
  assert.equal(open.player('b').health, 70);
});

test('positive pitch aims upward and can hit the head', () => {
  const sim = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -8 }, { id: 'b', nickname: 'B', x: 8, z: -15 }], { now: () => 4000 });
  sim.tick(3.1);
...
test('spawn protection belongs to the target rather than the shooter', () => {
  ...
  assert.equal(sim.player('b').health, 70, 'a protected shooter can still damage an unprotected target');
  sim.tick(0.2);
  sim.player('b').spawnProtection = 3;
  sim.fire('a', { sequence: 2, weapon: 'rifle', yaw: 0, pitch: -0.08, clientTime: 4000 });
  assert.equal(sim.player('b').health, 70, 'a protected target must not take damage');
});

test('applies death, score, respawn and match finish', () => {
  const sim = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -8 }, { id: 'b', nickname: 'B', x: 8, z: -15 }], { scoreLimit: 1, now: () => 5000 });
  sim.tick(3.1);
...
test('server bots produce movement or fire decisions', () => {
  const sim = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -10 }, { id: 'bot-1', nickname: 'BOT 01', x: 8, z: 0, isBot: true }], { difficulty: 'hard' });
...
test('time-limit ties have no winner', () => {
  const sim = new RifleFixture([{ id: 'a', nickname: 'A', x: 8, z: -8 }, { id: 'b', nickname: 'B', x: -8, z: 8 }], { matchSeconds: 0.05 });
  sim.tick(0.05);
  assert.equal(sim.finished, true);
