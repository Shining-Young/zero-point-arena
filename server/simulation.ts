import { advanceActor, lineClear, reloadAmmo, SPAWNS, type MovementInput, type MovementState } from '../lib/game/rules.ts';
import { MATCH_SECONDS, SCORE_LIMIT } from '../shared/config.ts';
...
export class MatchSimulation {
  ...
  readonly events: Array<{type:string;actorId:string;targetId?:string;value?:number}> = [];
  ...
  switchWeapon(id:string,weapon:Weapon){const p=this.player(id);if(!p.alive||p.reloadLeft>0)return false;p.weapon=weapon;const w=WEAPONS[weapon];p.ammo=Math.min(p.ammo,w.capacity);p.reserve=Math.min(p.reserve,w.reserve);p.cooldown=.25;return true;}
  ...
  fire(id:string,message:{sequence:number;weapon:Weapon;yaw:number;pitch:number;clientTime:number}){
    ...
    if(this.finished||!shooter.alive||shooter.cooldown>0||shooter.reloadLeft>0||shooter.ammo<=0||message.weapon!==shooter.weapon)return false;
    shooter.ammo-=1;shooter.cooldown=weapon.cadence;this.events.push({type:'shot',actorId:id});
    const dir={x:Math.sin(message.yaw)*Math.cos(message.pitch),z:Math.cos(message.yaw)*Math.cos(message.pitch)};
    let best:SimPlayer|undefined,bestAlong=Infinity;
    for(const target of this.players.values()){
      if(target.id===id||!target.alive)continue;
      const dx=target.x-shooter.x,dz=target.z-shooter.z,along=dx*dir.x+dz*dir.z;
      if(along<=0||along>weapon.range)continue;
      const perpendicular=Math.abs(dx*dir.z-dz*dir.x);
      ...
      if(!lineClear({x:shooter.x,y:1.55,z:shooter.z},{x:tx,y:1.2,z:tz}))continue;
      best=target;bestAlong=along;
    }
    if(best&&shooter.spawnProtection<=0){
      const headshot=message.pitch < -.08;
      const damage=headshot?weapon.damage*4:weapon.damage;
      best.health=Math.max(0,best.health-damage);this.events.push({type:headshot?'headshot':'hit',actorId:id,targetId:best.id,value:damage});
  ...
  tick(dt:number){
    ...
    for(const p of this.players.values()){
      ...
      if(p.reloadLeft>0){p.reloadLeft-=dt;if(p.reloadLeft<=0){const w=WEAPONS[p.weapon],loaded=reloadAmmo(p.ammo,p.reserve,w.capacity);p.ammo=loaded.ammo;p.reserve=loaded.reserve;}}
    ...
    if(this.remainingSeconds<=0){this.finished=true;this.winnerId=[...this.players.values()].sort((a,b)=>b.kills-a.kills)[0]?.id;}
  }

  snapshot():SnapshotEntity[]{return [...this.players.values()].map(p=>({id:p.id,nickname:p.nickname,isBot:p.isBot,x:p.x,y:p.y,z:p.z,yaw:p.yaw,pitch:p.pitch,health:p.health,weapon:p.weapon,ammo:p.ammo,reserve:p.reserve,kills:p.kills,deaths:p.deaths,alive:p.alive}));}
  ...
  private updateBot(bot:SimPlayer,dt:number){
    ...
    if(!target)return;const dx=target.x-bot.x,dz=target.z-bot.z,dist=Math.hypot(dx,dz);bot.yaw=Math.atan2(dx,dz);
  ...
  private respawn(p:SimPlayer){const index=[...this.players.keys()].indexOf(p.id)%SPAWNS.length,spawn=SPAWNS[index];Object.assign(p,{x:spawn.x,z:spawn.z,y:0,velocityY:0,grounded:true,health:100,alive:true,ammo:WEAPONS[p.weapon].capacity,reserve:WEAPONS[p.weapon].reserve,spawnProtection:3,respawnLeft:0});this.events.push({type:'respawn',actorId:p.id});}
  private makePlayer(p:Participant,index:number):SimPlayer{return {...p,isBot:Boolean(p.isBot),y:0,velocityY:0,grounded:true,crouched:false,yaw:0,pitch:0,health:100,alive:true,kills:0,deaths:0,weapon:'rifle',ammo:30,reserve:120,lastInput:-1,input:{sequence:-1,moveX:0,moveZ:0,yaw:0,pitch:0,jump:false,crouch:false,sprint:false,clientTime:0},cooldown:0,reloadLeft:0,respawnLeft:0,spawnProtection:3,x:p.x??SPAWNS[index].x,z:p.z??SPAWNS[index].z};}
}
